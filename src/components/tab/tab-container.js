...
export default function TabContainer() {
    ...
    if(pathname === '/diary/monthly-report' ||
    ...
    if (
        ...
        { label: '주간 분석서', href: '/diary/weekly-report', title: '분석서 조회' },
        { label: '월간 분석서', href: '/diary/monthly-report', title: '분석서 조회' },
        ]
    } else if (
        ...
        { label: '상담 신청서 조회', href: '/mypage/contact', title: '마이 페이지' },
    ...
    return (
        <>
        <Stack direction='row' justifyContent='space-between' sx={{ml:11,mr:17 ,mt:6, mb:3}}>
            <Typography variant='h2'>
                {tabProps[tabNum].title}
            </Typography>
            {
                pathCheck &&
                <Button 
        ...
        <Grid container spacing={2} sx={{mx:10}}>
            ...
            {pathCheck &&
            <Stack direction='row' justifyContent='flex-end' spacing={3} sx={{mr:17, mb:5}}>
                <DiaryCardArray />
                <DiaryDatePicker />
            </Stack>
            }

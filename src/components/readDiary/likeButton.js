...
function LikeButton({ diaryId }) {
  ...
  const handleClick = () => {
    if (liked) {
      // 이미 좋아요가 눌려있다면 취소 요청 보내기

      setLiked(false) //test용 문장임. 추후 백엔드 연동되면 이 줄 지우기

      axios
        .post(`/${diaryId}/LikeDiary`, { like: false })
        .then((response) => {
      ...
      // 좋아요 누르지 않은 경우 좋아요 요청 보내기

      setLiked(true) //test용 문장임. 추후 백엔드 연동되면 이 줄 지우기

      axios
        .post(`/${diaryId}/LikeDiary`, { like: true })
        .then((response) => {

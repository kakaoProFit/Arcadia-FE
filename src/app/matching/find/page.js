...
export default function AvatarList() {
  ...
  const filteredAvatars = avatars.filter((avatar) => {
    const regionMatch = selectedRegion === '전체' || avatar.region === selectedRegion;
    const symptomMatch = selectedSymptom === '전체' || avatar.symptom === selectedSymptom;
  ...
  const visibleAvatars = filteredAvatars.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className={styles.main}>
      ...
      <Grid container spacing={2} alignItems="center" justifyContent="center" sx = {{ mt: 6 }}>
        <Grid item>
          <IconButton onClick={handlePrevPage}><ArrowBack /></IconButton>
        </Grid>
          ...
          <Grid key={index} item>
            <Card>
              <CardContent>
                ...
                <Grid item textAlign="center" alignItems="center" sx={{ mt: 4 }}>
        ...
        <Grid item>
          <IconButton onClick={handleNextPage}><ArrowForward /></IconButton>
        </Grid>
      </Grid>
      </Grid>
    </div>
